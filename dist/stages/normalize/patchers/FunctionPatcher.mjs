        ...
        try {
            for (var _c = __values(this.parameters.entries()), _d = _c.next(); !_d.done; _d = _c.next()) {
                ...
                if (firstRestParamIndex === -1 || i < firstRestParamIndex) {
                    ...
                    assignments.push.apply(assignments, __spread(newAssignments));
                    neededExplicitBindings.push.apply(neededExplicitBindings, __spread(newBindings));
                }
                else {
                    parameter.patch();
                }
        ...
        if (firstRestParamIndex !== -1) {
            if (firstRestParamIndex === this.parameters.length - 1 &&
                ...
                for (var i = firstRestParamIndex; i < this.parameters.length; i++) {
                    neededExplicitBindings.push.apply(neededExplicitBindings, __spread(getAssigneeBindings(this.parameters[i].node)));
                }
            }
        }
        var uniqueExplicitBindings = __spread(new Set(neededExplicitBindings));
    ...
    FunctionPatcher.prototype.patchParameterAndGetAssignments = function (parameter) {
        ...
        this.addDefaultParamAssignmentAtScopeHeader = function (assigneeCode, initCode, assigneeNode) {
            if (assigneeNode.type === 'Identifier' || assigneeNode.type === 'MemberAccessOp') {
                ...
                newBindings.push.apply(newBindings, __spread(getAssigneeBindings(assigneeNode)));
                return paramName;
            }
        };
        parameter.patch();
        ...
        return {
            newAssignments: __spread(defaultParamAssignments, thisAssignments),
